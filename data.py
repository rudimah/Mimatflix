...
def load_data(limit=None):
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        query = "SELECT id_movie, title, poster, url, movie_url, download, status, created_at, updated_at FROM movie WHERE status IN ('available', 'deleted') ORDER BY updated_at DESC, id_movie DESC"
        if limit:
        ...
        for m in movies:
            ...
            if not m.get('movie_url') and m.get('status') == 'available':
                m['status'] = 'deleted'
                m['download'] = False
...
def update_movie(id_movie, movie):
    ...
    try:
        ...
        update_query = (
            "UPDATE movie SET title = %s, poster = %s, url = %s, movie_url = %s, download = %s, source_link = %s, status = %s "
            "WHERE id_movie = %s"
        )
...
def delete_video_by_id(id_movie):
    ...
    try:
        ...
        update_query = "UPDATE movie SET movie_url = NULL, download = FALSE, status = 'deleted' WHERE id_movie = %s"
